...
[xml]$xaml=@'
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" Title="Codex Pulse" Width="320" Height="245" MinWidth="280" MinHeight="190" WindowStyle="None" AllowsTransparency="True" Background="Transparent" Topmost="True" ResizeMode="CanResizeWithGrip" ShowInTaskbar="True">
 <Border Background="#0D1113" BorderBrush="#293438" BorderThickness="1" CornerRadius="13">
  <Grid><Grid.RowDefinitions><RowDefinition Height="42"/><RowDefinition Height="*"/><RowDefinition Height="34"/></Grid.RowDefinitions>
   <Border Grid.Row="0" BorderBrush="#293438" BorderThickness="0,0,0,1" Name="TitleBar"><Grid Margin="11,0"><Grid.ColumnDefinitions><ColumnDefinition/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions>
    <StackPanel Orientation="Horizontal" VerticalAlignment="Center"><Border BorderBrush="#98F7C8" BorderThickness="1.5" CornerRadius="6" Width="23" Height="23"><TextBlock Text="C" Foreground="#98F7C8" FontSize="10" FontWeight="Bold" HorizontalAlignment="Center" VerticalAlignment="Center"/></Border><StackPanel Margin="7,0,0,0" VerticalAlignment="Center"><TextBlock Text="CODEX PULSE" Foreground="#F1F6F3" FontSize="9" FontWeight="Bold"/><TextBlock Name="Status" Text="CONNECTING" Foreground="#7F8C89" FontSize="7"/></StackPanel></StackPanel>
    <StackPanel Grid.Column="1" Orientation="Horizontal"><Button Name="Compact" Content="-" Width="25" Background="Transparent" BorderThickness="0" Foreground="#7F8C89" FontSize="14"/><Button Name="Close" Content="X" Width="25" Background="Transparent" BorderThickness="0" Foreground="#7F8C89" FontSize="11"/></StackPanel>
   </Grid></Border>
   <ScrollViewer Grid.Row="1" VerticalScrollBarVisibility="Auto"><StackPanel Margin="11,8,11,7">
    <Grid Margin="0,0,0,4"><StackPanel Orientation="Horizontal"><TextBlock Name="Plan" Text="Detecting plan..." Foreground="#F1F6F3" FontSize="14" FontWeight="SemiBold"/><TextBlock Text="  LIVE" Foreground="#98F7C8" FontSize="7" FontWeight="Bold" VerticalAlignment="Center"/></StackPanel></Grid>
    <StackPanel Name="Limits"/>
    <Border BorderBrush="#293438" BorderThickness="0,1,0,0" Margin="0,5,0,0" Padding="0,5,0,0" Name="Telemetry"><Grid><Grid.ColumnDefinitions><ColumnDefinition/><ColumnDefinition/><ColumnDefinition/></Grid.ColumnDefinitions><StackPanel><TextBlock Text="PROCESSED" Foreground="#7F8C89" FontSize="7"/><TextBlock Name="TotalTokens" Text="-" Foreground="#F1F6F3" FontSize="10" FontWeight="Bold"/></StackPanel><StackPanel Grid.Column="1"><TextBlock Text="CACHED" Foreground="#7F8C89" FontSize="7"/><TextBlock Name="CachedTokens" Text="-" Foreground="#F1F6F3" FontSize="10" FontWeight="Bold"/></StackPanel><StackPanel Grid.Column="2"><TextBlock Text="EST. LEFT" Foreground="#7F8C89" FontSize="7"/><TextBlock Name="Estimate" Text="Calibrating..." Foreground="#FFAD55" FontSize="10" FontWeight="Bold"/></StackPanel></Grid></Border>
    <TextBlock Name="Notice" Foreground="#D7B980" FontSize="7" TextWrapping="Wrap" MaxHeight="22" Margin="0,4,0,0"/>
   </StackPanel></ScrollViewer>
   <Border Grid.Row="2" BorderBrush="#293438" BorderThickness="0,1,0,0"><Grid Margin="11,0"><StackPanel Orientation="Horizontal" VerticalAlignment="Center"><TextBlock Name="Source" Text="LOCAL-FIRST" Foreground="#7F8C89" FontSize="7"/><TextBlock Name="Updated" Text=" · Never updated" Foreground="#7F8C89" FontSize="7"/></StackPanel><Button Name="Refresh" Content="R" ToolTip="Refresh now (automatic every minute)" HorizontalAlignment="Right" VerticalAlignment="Center" Width="25" Height="23" Background="Transparent" BorderBrush="#344044" Foreground="#F1F6F3" FontSize="9"/></Grid></Border>
  </Grid>
 </Border>
</Window>
'@
...
function New-LimitCard($Limit){$color=if($Limit.UsedPercent-ge90){'#FF756F'}elseif($Limit.UsedPercent-ge70){'#FFAD55'}else{'#98F7C8'};$card=[Windows.Controls.Border]::new();$card.Background='#141A1D';$card.BorderBrush='#293438';$card.BorderThickness=1;$card.CornerRadius=7;$card.Padding=7;$card.Margin='0,2,0,2';$stack=[Windows.Controls.StackPanel]::new();$top=[Windows.Controls.Grid]::new();$left=[Windows.Controls.StackPanel]::new();$a=[Windows.Controls.TextBlock]::new();$a.Text=$Limit.Label;$a.Foreground='#F1F6F3';$a.FontSize=9;$a.FontWeight='SemiBold';$b=[Windows.Controls.TextBlock]::new();$b.Text=$Limit.Detail.ToUpperInvariant();$b.Foreground='#7F8C89';$b.FontSize=7;[void]$left.Children.Add($a);[void]$left.Children.Add($b);$pct=[Windows.Controls.TextBlock]::new();$pct.Text="$([math]::Round($Limit.RemainingPercent))% LEFT";$pct.Foreground=$color;$pct.FontSize=12;$pct.FontWeight='Bold';$pct.HorizontalAlignment='Right';[void]$top.Children.Add($left);[void]$top.Children.Add($pct);$bar=[Windows.Controls.ProgressBar]::new();$bar.Value=$Limit.RemainingPercent;$bar.Maximum=100;$bar.Height=5;$bar.Foreground=$color;$bar.Background='#263034';$bar.Margin='0,5,0,4';$foot=[Windows.Controls.Grid]::new();$used=[Windows.Controls.TextBlock]::new();$used.Text="$([math]::Round($Limit.UsedPercent))% USED";$used.Foreground='#7F8C89';$used.FontSize=7;$reset=[Windows.Controls.TextBlock]::new();$reset.Text=Format-Countdown $Limit.ResetAt;$reset.Foreground='#B7C2BF';$reset.FontSize=7;$reset.HorizontalAlignment='Right';[void]$foot.Children.Add($used);[void]$foot.Children.Add($reset);[void]$stack.Children.Add($top);[void]$stack.Children.Add($bar);[void]$stack.Children.Add($foot);$card.Child=$stack;$card}
...
$window.Add_PreviewMouseLeftButtonDown({param($sender,$eventArgs)$node=$eventArgs.OriginalSource;while($node-and$node-ne$window){if($node-is[Windows.Controls.Primitives.ButtonBase]-or$node-is[Windows.Controls.Primitives.ScrollBar]){return};$node=[Windows.Media.VisualTreeHelper]::GetParent($node)};try{$window.DragMove()}catch{}});$Close.Add_Click({$window.Hide()});$Refresh.Add_Click({Refresh-Ui});$compactMode=$false;$Compact.Add_Click({$script:compactMode=!$script:compactMode;if($script:compactMode){$Telemetry.Visibility='Collapsed';$Notice.Visibility='Collapsed';$window.Height=165}else{$Telemetry.Visibility='Visible';$Notice.Visibility='Visible';$window.Height=245}})
